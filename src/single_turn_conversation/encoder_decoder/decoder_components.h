...
struct DecoderComponents {
    ...
    vector<DynamicGRUBuilder> decoders;
    ...
    void forward(Graph &graph, const HyperParams &hyper_params,
            const vector<GRUParams*> &decoder_params,
            AdditiveAttentionParams &attention_params,
        ...
        Node *guide = decoders.front().size() == 0 ?
            ...
            static_cast<Node*>(decoders.front().hiddens.at(decoders.front().size() - 1));
        ...
        if (decoder_params.size() > 1) {
            ...
            for (int i = 1; i < decoder_params.size(); ++i) {
                ...
                if (i == 1) {
                    sum_inputs = {clip, decoders.at(i - 1).hiddens.back()};
                } else {
                    sum_inputs = {decoders.at(i - 1).hiddens.back(),
                        decoders.at(i - 2).hiddens.back()};
                }
    ...
    Node *decoderToWordVectors(Graph &graph, const HyperParams &hyper_params,
        ...
        vector<Node *> concat_inputs = {
            contexts.at(i), decoders.back().hiddens.at(i), i == 0 ?
                bucket(to_word_params.W.inDim() - 2 * hyper_params.hidden_dim, graph) :
